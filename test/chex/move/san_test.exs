defmodule Chex.Move.SANTest do
  ...
  describe "pawn moves" do
    ...
    test "underpromotion with capture and check" do
      {:ok, game} = Chex.Game.new("2k2r2/4P3/8/8/8/8/2K5/8 w - - 0 1")
      assert {{:e, 7}, {:f, 8}, :rook} = Move.parse("exf8=R+", game)
    end
  end
  ...
  describe "non-pawn moves" do
    test "rook" do
      ...
      assert {{:h, 1}, {:h, 5}} = Move.parse("Rh5", game)
      assert {{:a, 8}, {:d, 8}} = Move.parse("Rd8", %{game | active_color: :black})
    end

    test "rook with check" do
      {:ok, game} = Chex.Game.new("r3k3/8/8/8/8/8/8/4K2R w - - 0 1")
      assert {{:h, 1}, {:h, 8}} = Move.parse("Rh8+", game)
      assert {{:a, 8}, {:a, 1}} = Move.parse("Ra1+", %{game | active_color: :black})
    end

    test "knight" do
      {:ok, game} = Chex.Game.new()
      assert {{:g, 1}, {:f, 3}} = Move.parse("Nf3", game)
    end

    test "knight with check" do
      {:ok, game} = Chex.Game.new("4k3/8/8/3Nn3/8/8/8/4K3 w - - 0 1")
      assert {{:d, 5}, {:c, 7}} = Move.parse("Nc7+", game)
      assert {{:e, 5}, {:f, 3}} = Move.parse("Nf3+", %{game | active_color: :black})
    end

    test "bishop" do
      {:ok, game} = Chex.Game.new("2b1k3/8/8/8/8/8/8/2B1K3 w - - 0 1")
      assert {{:c, 1}, {:g, 5}} = Move.parse("Bg5", game)
      assert {{:c, 8}, {:g, 4}} = Move.parse("Bg4", %{game | active_color: :black})
    end

    test "queen" do
      {:ok, game} = Chex.Game.new("3qk3/8/8/8/8/8/8/3QK3 w - - 0 1")
      assert {{:d, 1}, {:a, 4}} = Move.parse("Qa4", game)
    end

    test "king" do
      {:ok, game} = Chex.Game.new("4k3/8/8/8/8/8/8/4K3 w - - 0 1")
      assert {{:e, 1}, {:d, 2}} = Move.parse("Kd2", game)
    end
  end
  ...
  describe "castling" do
    ...
    test "white kingside", %{game: game} do
      assert {{:e, 1}, {:g, 1}} = Move.parse("O-O", game)
    end

    test "white queenside", %{game: game} do
      assert {{:e, 1}, {:c, 1}} = Move.parse("O-O-O", game)
    end

    test "black kingside", %{game: game} do
      assert {{:e, 8}, {:g, 8}} = Move.parse("O-O", %{game | active_color: :black})
    end

    test "black queenside", %{game: game} do
      assert {{:e, 8}, {:c, 8}} = Move.parse("O-O-O", %{game | active_color: :black})
    end
  end
  ...
  describe "ambiguous moves" do
    ...
    test "bishop differentiated by file", %{game: game} do
      ...
      assert {{:d, 6}, {:b, 8}} = Move.parse("Bdb8", game)
      assert {{:a, 7}, {:b, 8}} = Move.parse("Bab8", game)
    end

    test "rook differentiated by file", %{game: game} do
      game = %{game | active_color: :black}
      assert {{:d, 8}, {:f, 8}} = Move.parse("Rdf8", game)
      assert {{:h, 8}, {:f, 8}} = Move.parse("Rhf8", game)
    end

    test "differentiated by rank", %{game: game} do
      assert {{:a, 1}, {:a, 3}} = Move.parse("R1a3", game)
      assert {{:a, 5}, {:a, 3}} = Move.parse("R5a3", game)
    end

    test "differentiated by file and rank", %{game: game} do
      assert {{:h, 4}, {:e, 1}} = Move.parse("Qh4e1", game)
    end
  end
end
